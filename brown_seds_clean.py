...
def calc_w3_w4(redshift, given_name,projpath):
    ...
    print(f"w3minusw4_unc: {w3minusw4_unc}")
...
def choose_dataset(dataset_name):
    ...
    This will return all the necessary variables
    """
...
with PdfPages(name) as pdf:
    ...
    for i in range(0, len(files)):
        ...
        df = pd.read_fwf(StringIO(table))
        print(df)

        '''
        wave = np.array(table['col1'])
        print("wave:", wave)
        fnu = np.array(table['col2'])
        ...
        for j in range(0, len(zarray)):
            ...
            model_mags = observate.getSED(wave * (1 + zarray[j]), fnu, filterlist=wise_filters)
