...
def mag_to_flux(name, w2= False):
	...
	if w2:
		flux_two = w_two_const * (10 ** (-(float(t['w2_mag']) / 2.5)))
		flux_two_err = w_two_const * np.log(10) / 2.5 * (10 ** (-(float(t['w2_mag']) / 2.5))) * float(
			t['w2_mag_err'])
		return [flux_two,flux_two_err, flux_three,flux_three_err, flux_four, flux_four_err]
